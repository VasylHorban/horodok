...
import { trimRichTextByCharacters } from '~/utils/contentful';
...
const Content = ({
  ...
  const maxChars = 500; // Limit to 200 characters
  const trimmedContent = trimRichTextByCharacters(content, maxChars);
  return (
    <WidgetWrapper
      id={id ? id : ''}
          ...
          <div className="self-center md:basis-1/2">
            {content && (
              <div className="mb-8 lg:mb-12 text-lg text-gray-600 dark:text-slate-400">
                {documentToReactComponents(trimmedContent)}
              </div>
            )}
            <ItemGrid
              items={items}
          ...
          <div aria-hidden="true" className="mt-10 md:mt-0 md:basis-1/2">
            {image && (
              <div className="relative m-auto max-w-4xl">
                <Image
                  className="mx-auto w-full rounded-lg shadow-lg bg-gray-400 dark:bg-slate-700 max-h-[500]"
                  src={image.src}
                  width={828}
                  height={500}
                  alt={image.alt}
